		...
		try (Connection conn = DBUtil.getConnection(); PreparedStatement pstmt = conn.prepareStatement(orderSql)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				...
				String itemSql = "SELECT oi.order_item_id, oi.order_id, oi.menu_id, "
						+ "oi.quantity, oi.unit_price, m.menu_name, c.category_name " + "FROM ORDER_ITEM oi "
						+ "JOIN MENU m ON oi.menu_id = m.menu_id " + "JOIN CATEGORY c ON m.category_id = c.category_id "
						+ "WHERE oi.order_id = ?";
				...
				try (PreparedStatement itemPs = conn.prepareStatement(itemSql)) {
					...
					try (ResultSet itemRs = itemPs.executeQuery()) {
						while (itemRs.next()) {
							list.add(new OrderItem(itemRs.getLong("order_item_id"), itemRs.getLong("order_id"),
									itemRs.getLong("menu_id"), itemRs.getInt("quantity"), itemRs.getInt("unit_price"),
									itemRs.getString("menu_name"), itemRs.getString("category_name")));
						}
					}
				}
			}
