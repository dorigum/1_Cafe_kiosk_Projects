...
public class OrderingView {
	...
	public void runWithCart(MenuController menuController, Member member, List<OrderItem> initialCart) {
		List<OrderItem> cart = new ArrayList<>(initialCart);
	...
	private boolean runMenuSelectionLoop(MenuController menuController, List<Menu> menus, List<OrderItem> cart,
			Member member) {
	...
	private OrderItem copyOrderItemWithQuantity(OrderItem orderItem, int quantity) {
		return new OrderItem(orderItem.getOrderItemId(), orderItem.getOrderId(), orderItem.getMenuId(), quantity,
				orderItem.getUnitPrice(), orderItem.getMenuNameSnapshot(), orderItem.getCategoryNameSnapshot(),
